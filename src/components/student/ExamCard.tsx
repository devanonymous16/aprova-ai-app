      ...
      <CardHeader className="pb-2">
        <div className="flex justify-between items-start">
          <div>
            ...
            <p className="text-muted-foreground text-sm">{examData.organization}</p>
          </div>
      ...
      <CardContent className="flex-grow">
        <div className="grid grid-cols-2 gap-2 text-sm mb-4">
          <div>
            ...
            <p className="font-medium">{examData.vacancy_count || "-"}</p>
          </div>
          <div>
            <p className="text-muted-foreground">Salário</p>
            <p className="font-medium">
              {examData.salary 
                ? new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(examData.salary)
                : "-"
              }
            </p>
          </div>
        ...
        {isStudentExam && (
          <div className="mb-4">
            <div className="flex justify-between text-sm mb-1">
              <span>Progresso</span>
              <span>{(exam as StudentExam).progress_percentage}%</span>
            </div>
            <Progress value={(exam as StudentExam).progress_percentage} className="h-2" />
          </div>
        )}
