...
export const StudentDetailModal: React.FC<StudentDetailModalProps> = ({ student, isOpen, onOpenChange }) => {
  const studentId = student?.id ?? null; // Extrai o ID para logs
  const mockExamPositionId = 'pos-123-abc'; // Placeholder

  // Log inicial ao renderizar/atualizar o modal
  ...
  const {
      ...
      status: performanceStatus, // Adiciona status para mais detalhes
  } = useStudentPerformanceData(studentId, mockExamPositionId);

  // Log do estado do hook de performance
  console.log('[Modal Performance Hook State]', {
      studentId,
      mockExamPositionId,
      status: performanceStatus, // 'pending', 'error', 'success'
   ...
   }, [studentId]); // Dependência apenas no ID do aluno
      ...
      <DialogContent className="sm:max-w-[800px] md:max-w-[900px] lg:max-w-[1000px] max-h-[90vh] flex flex-col">
        ...
        <div className="flex-grow overflow-y-auto pr-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4 mb-6 border-b pb-6">
                {/* ... Nome, Email, Membro Desde, Cargo Alvo ... */}
            ...
            <div className="space-y-6">
                ...
                <div className="p-4 border rounded-lg">
                    <h3 className="text-lg font-semibold mb-3 flex items-center gap-2">
                     ...
                     {console.log('[Modal Render Logic] Checking conditions:', { isLoadingPerformance, errorPerformance })}
                     ...
                     {errorPerformance && !isLoadingPerformance && ( // Mostra erro apenas se não estiver carregando
                     ...
                     {!isLoadingPerformance && !errorPerformance && (
                        <>
                            ...
                            {console.log('[Modal Render Logic] Rendering PerformanceTopicTable with data length:', performanceData?.length)}
                            <PerformanceTopicTable data={performanceData ?? []} />
                        </>
                     )}
                </div>

                 {/* Placeholder Comparativo com a Média (sem alterações) */}
                 <div className="p-4 border rounded-lg bg-gray-50">
                  ...
                  {/* Placeholder Alertas e Riscos (sem alterações) */}
